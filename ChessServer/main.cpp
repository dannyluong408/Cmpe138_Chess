...
void handle_message(char *buffer, const int length, Session *session) {
    ...
    if ( strcmp( arg[0] , "query") == 0 && (count == 2 || count == 3 || count == 4)){
        ...
        if ( strcmp (arg[1] , "stats") == 0 && count == 3){  // query stats "name"
            ...
            stat_query << "SELECT * FROM player WHERE player_id = "  << mysqlpp::quote << arg[2]; // Sends Query
            try {
        ...
        userquery:
            ...
            user_query << "SELECT id, email, active_lobby FROM user_info ORDER BY active_lobby";
            try {
        ...
        historyquery:
            ...
            try {
                ...
                bufferPos += snprintf(&queryBuffer[bufferPos], sizeof(queryBuffer)-bufferPos, "%12s %12s %12s %12s %12s %16s %20s %12s\n",
                    "match_id", "avg_movetime", "match_length", "replay_id", "elo_change", "winner", "date_time", "l_id");

                for (int x = 0; x < result.num_rows(); x++) {
                    bufferPos += snprintf(&queryBuffer[bufferPos], sizeof(queryBuffer)-bufferPos, "%12i %12s %12s %12i %12i %16s %20s %12i\n", (int)(result[x]["match_id"]),
